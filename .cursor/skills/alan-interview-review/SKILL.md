...
Only for `fit`, `screening`, and `past-project` sessions. Read the files in `profile/` (chataigne.txt, opensourcetogether.txt, s37.txt, cursor-ambassador.txt) and produce:
...
If not already done this session, produce a full mapping of the user's 4 main experiences (Châtaigne, OpenSource Together, S37, Cursor Ambassador) to Alan's 4 values. For each value, assign the strongest project and the strongest specific moment from that project.
